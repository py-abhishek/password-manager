...
class PasswordManagerAPI:
    ...
    def register_routes(self):
        ...
        def add_password(data: PasswordData):
            ...
            print("add password")
            if self.db.add_password(data.platform, data.username, data.password):
        ...
        def search_password(search_term: str):
            result = self.db.search_password(search_term)
            if result:
                return result
        ...
        def update_password(id: int, data: PasswordData):
            if self.db.update_password(id, data.platform, data.username, data.password):
